...
public class AreaPickerView extends Dialog {
     ...
     * 市
     */
    private List<AddressDistrictBean.CityBean> cityBeans;
    /**
     * 区
     */
    private List<AddressDistrictBean.CityBean.AreaBean> areaBeans;
    ...
    public AreaPickerView(@NonNull Context context, int themeResId, List<AddressDistrictBean> addressDistrictBeans) {
        super(context, themeResId);
        this.addressDistrictBeans = addressDistrictBeans;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        provinceAdapter = new ProvinceAdapter(R.layout.item_address, addressDistrictBeans);
        ...
        provinceAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                cityBeans.clear();
                areaBeans.clear();
                addressDistrictBeans.get(position).setStatus(true);
                provinceSelected = position;
                if (oldProvinceSelected != -1 && oldProvinceSelected != provinceSelected) {
                    addressDistrictBeans.get(oldProvinceSelected).setStatus(false);
                    Log.e("AreaPickerView", "清空");
                }
                if (position != oldProvinceSelected) {
                    if (oldCitySelected != -1) {
                        addressDistrictBeans.get(oldProvinceSelected).getChildren().get(oldCitySelected).setStatus(false);
                    }
                    if (oldAreaSelected != -1) {
                        addressDistrictBeans.get(oldProvinceSelected).getChildren().get(oldCitySelected).getChildren().get(oldAreaSelected).setStatus(false);
                    }
                    oldCitySelected = -1;
                    oldAreaSelected = -1;
                }
                cityBeans.addAll(addressDistrictBeans.get(position).getChildren());
                provinceAdapter.notifyDataSetChanged();
                cityAdapter.notifyDataSetChanged();
                areaAdapter.notifyDataSetChanged();
                strings.set(0, addressDistrictBeans.get(position).getLabel());
                if (strings.size() == 1) {
                    strings.add("请选择");
    ...
    public void setSelect(int... value) {
        ...
        if (value == null) {
            ...
            if (isCreate) {
                ...
                if (provinceSelected != -1)
                    addressDistrictBeans.get(provinceSelected).setStatus(false);
                if (citySelected != -1)
                    addressDistrictBeans.get(provinceSelected).getChildren().get(citySelected).setStatus(false);
                cityBeans.clear();
                areaBeans.clear();
                provinceAdapter.notifyDataSetChanged();
                cityAdapter.notifyDataSetChanged();
        ...
        if (value.length == 3) {
            strings.add(addressDistrictBeans.get(value[0]).getLabel());
            strings.add(addressDistrictBeans.get(value[0]).getChildren().get(value[1]).getLabel());
            strings.add(addressDistrictBeans.get(value[0]).getChildren().get(value[1]).getChildren().get(value[2]).getLabel());
            tabLayout.setupWithViewPager(viewPager);
            viewPagerAdapter.notifyDataSetChanged();
            ...
            if (citySelected != -1)
                addressDistrictBeans.get(provinceSelected).getChildren().get(citySelected).setStatus(false);
            addressDistrictBeans.get(value[0]).setStatus(true);
            addressDistrictBeans.get(value[0]).getChildren().get(value[1]).setStatus(true);
            addressDistrictBeans.get(value[0]).getChildren().get(value[1]).getChildren().get(value[2]).setStatus(true);
            cityBeans.clear();
            cityBeans.addAll(addressDistrictBeans.get(value[0]).getChildren());
            areaBeans.clear();
            areaBeans.addAll(addressDistrictBeans.get(value[0]).getChildren().get(value[1]).getChildren());
            provinceAdapter.notifyDataSetChanged();
            cityAdapter.notifyDataSetChanged();
            areaAdapter.notifyDataSetChanged();

...
public class SelectorUtils {
     ...
     *            默认以及按下状态的颜色值
     */
    public static void changeViewColor(Context context,StateListDrawable drawable, int[] rgbColors) {
        ...
        try {
            ...
            for (int i = 0; i < drawables.length; i++) {
                ...
                StateListDrawable gd = (StateListDrawable) drawables[i];
                if (gd == null) {
                    break;
                }
                if (i == 0) {
                    // 我们对按下的状态做浅色处理
                    drawable.addState(new int[]{android.R.attr.state_checked}, ContextCompat.getDrawable(context, R.drawable.nomal_bottom));
                } else {
                    drawable.addState(new int[]{-android.R.attr.state_checked}, ContextCompat.getDrawable(context, R.drawable.nomal));
                    // 对默认状态做深色处理
                }
            }
